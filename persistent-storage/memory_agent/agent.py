...
def update_reminder(index: int, updated_text: str, tool_context: ToolContext) -> dict:
    ...
    if 0 <= index < len(reminders):
        reminders[index] = updated_text
        tool_context.state["reminders"] = reminders
        return {
            "action": "update_reminder",
            "index": index,
            "updated_text": updated_text,
            "message": f"Updated reminder at index {index} to: {updated_text}",
        }
    else:
        return {
            "action": "update_reminder",
            "error": f"Index {index} is out of range. Total reminders: {len(reminders)}"
        }
